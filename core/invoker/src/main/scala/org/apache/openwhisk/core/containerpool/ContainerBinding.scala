...
/**
 * An eXternal Actor, it represents an actor instance related to an Action invocation
 *
 ...
 */
class ContainerBinding(var actor: XActor)(implicit val logging: Logging) extends Serializable {
...
object ContainerBinding {
  ...
  def apply(actor: XActor)(implicit logging: Logging): ContainerBinding =
    invocationManagers.getOrElse(actor, {
      val manager = new ContainerBinding(actor)
      invocationManagers += (actor -> manager)
      logging.info(this, s"ContainerBinding initialized for $actor")

      manager
    })

  def apply(msg: ActivationMessage)(implicit logging: Logging): ContainerBinding = ContainerBinding(XActor(msg))

  implicit class ContainerDataChecker(data: ContainerData)(implicit actor: XActor, logging: Logging) {
    def canRunXActor: Boolean = apply(actor).checkContainerData(data)
  }
}
